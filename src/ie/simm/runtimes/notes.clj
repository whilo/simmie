(ns ie.simm.runtimes.notes
  ...
  (:require [ie.simm.languages.bindings :as lb]
            [ie.simm.languages.gen-ai :refer [cheap-llm cheap-llm]]
...
(defn notes
  ...
  (let [mi (mult in)
       ...
       ;; subscriptions for this runtime context
        ...
        _ (sub pi :ie.simm.languages.notes/related-notes related-ch)
      ...
      (go-loop-try S [{[conn conv] :args :as m} (<? S related-ch)]
                   ...
                   (when m
                     (let [all-links (d/q '[:find [?t ...] :where [_ :conversation/link ?t]] @conn)
                       ...
                       (put? S out (assoc m
                                          :type :ie.simm.languages.notes/related-notes-reply
